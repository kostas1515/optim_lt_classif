...
def load_cifar(args):
    cwd = os.getenv('owd')
    root = os.path.join(cwd,args.dataset.root)
    auto_augment_policy = getattr(args.augmentations.auto_augment, "auto_augment", None)
    random_erase_prob = getattr(args.augmentations.random_erase, "random_erase", 0.0)
    ra_magnitude = args.augmentations.ra_magnitude
    augmix_severity = args.augmentations.augmix_severity
    ...
    if args.dataset.dset_name == 'cifar10':
        train_dataset = IMBALANCECIFAR10(root=root,imb_type=args.dataset.imb_type, imb_factor=args.dataset.imb_factor,
                                                        rand_number=args.dataset.rand_number, train=True, download=True, transform=transform_train)
        val_dataset = datasets.CIFAR10(root=root, train=False, download=True, transform=transform_val)
    elif args.dataset.dset_name == 'cifar100':
        train_dataset = IMBALANCECIFAR100(root=root,imb_type=args.dataset.imb_type, imb_factor=args.dataset.imb_factor,
                                                        rand_number=args.dataset.rand_number, train=True, download=True, transform=transform_train)
        val_dataset = datasets.CIFAR100(root=root, train=False, download=True, transform=transform_val)
